...
class BinaryReader {
 public:
  ...
  [[nodiscard]] std::uint8_t read_u8() { return read_scalar<std::uint8_t>(); }
  [[nodiscard]] std::uint16_t read_u16() { return read_scalar<std::uint16_t>(); }
  ...
  [[nodiscard]] float read_f32() { return std::bit_cast<float>(read_scalar<std::uint32_t>()); }
  ...
  [[nodiscard]] std::size_t tell() const { return m_position; }
...
inline void BinaryReader::skip(const std::size_t count) {
  ...
  if (count > remaining()) {
    m_has_error = true;
    m_error = fmt::format("skip of {} bytes at offset {} exceeds buffer size {}",
                          count,
                          m_position,
                          m_data.size());
