...
std::expected<std::unique_ptr<ModelScene>, std::string> ModelScene::create_from_scenario_manager(
  ...
  ScenarioRuntime* runtime{manager->scenario_runtime()};
  if (runtime == nullptr) {
    return std::expected<std::unique_ptr<ModelScene>, std::string>{
...
void ModelScene::update(const float delta_time, const Input::InputManager& input) {
  ...
  // (camera above) → set the listener → run the script tick (queues
  // play/stop) → update the audio system (drain callbacks, start queued
  // voices, spatialize).
  ...
  // Advance script execution with the real application delta in seconds. The
  // runtime converts seconds to 30 Hz script-frame units (clamped to three
  // frames), matching the value-pool durations.
  if (m_runtime != nullptr) {
    m_runtime->tick(delta_time);
  }
