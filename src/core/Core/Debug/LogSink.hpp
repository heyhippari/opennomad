...
class RingBufferSink final : public spdlog::sinks::base_sink<std::mutex> {
 public:
  ...
  /// One buffered log line with its severity and category, for display in
  /// the UI. Category is retained as metadata so the viewer never has to
  /// parse it back out of the formatted line.
  struct LogEntry {
    std::string line;
  ...
  /// Return a snapshot of all buffered lines with their levels and categories
  /// (oldest first).
  [[nodiscard]] std::vector<LogEntry> get_entries() const {
    ...
    const auto entry_at{[this, count](const std::size_t i) {
      ...
      return LogEntry{m_buffer.at(idx), m_levels.at(idx), m_categories.at(idx)};
    }};
 ...
 private:
  ...
  std::array<LogCategory, Capacity> m_categories{};
